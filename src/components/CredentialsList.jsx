...
const filterData = (data, keyword) => {
  ...
  return data.filter((obj) => {
    ...
    const user = obj.user || obj.User ? (obj.user || obj.User).toLowerCase() : "";

    return service.includes(lowerCaseKeyword) || user.includes(lowerCaseKeyword);
  });
};
  ...
  return (
    <div className="creds-container">
      ...
      <div className="search-wrapper relative">
        ...
        <i
          tabIndex="-1"
          className="bi bi-search absolute inset-y-0 right-3 flex items-center text-gray-600 dark:text-gray-400 transition duration-300"
        ></i>
      </div>
        ...
        {credentials.length > 0 ? (
          <div>
            {filterData(credentials, searchKeyword).length > 0 ? (
              filterData(credentials, searchKeyword).map((credential, index) => (
                <div
